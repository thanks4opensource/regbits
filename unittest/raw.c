...
__attribute__((noinline)) void zero()
{
    ...
    *(TIMER1  + TIMER_PRESCALE_OFFSET)    = 0;
     (GPIO1   + GPIO_BYTES_OFFSET    )[3] = 0;
    *(GPIO1   + GPIO_SET_OFFSET      )    = 0;
}
...
{
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) &= ~SERIAL_CONFIG_DATALEN_MSK;
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) &= ~SERIAL_CONFIG_DATALEN_MSK;
}
...
__attribute__((noinline)) void cmp_zero()
{
    ...
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = 0;
    ...
    if (*(SERIAL2 + SERIAL_CONFIG_OFFSET) == 0)
        *(TIMER1 + TIMER_PRESCALE_OFFSET) = 43 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET) = 17 << TIMER_PRESCALE_PRESCALER_POS;
...
__attribute__((noinline)) void cmp_bits()
{
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = SERIAL_CONFIG_POLARITY;
    ...
    if (*(SERIAL2 + SERIAL_CONFIG_OFFSET) == SERIAL_CONFIG_POLARITY)
        *(TIMER1 + TIMER_PRESCALE_OFFSET) = 11 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET) = 67 << TIMER_PRESCALE_PRESCALER_POS ;
...
__attribute__((noinline)) void cmp_reg()
{
    ...
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = word;
    ...
    if (*(SERIAL2 + SERIAL_CONFIG_OFFSET) == word)
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 223 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 241 << TIMER_PRESCALE_PRESCALER_POS;
...
__attribute__((noinline)) void bytes()
{
     (GPIO1 + GPIO_BYTES_OFFSET)[3] = 1     ;
    *(GPIO1 + GPIO_SET_OFFSET  )    = 1 << 3;
}
...
__attribute__((noinline)) void runtime_bits_array()
{
    volatile unsigned   index = 3;
     ...
     (GPIO1 + GPIO_BYTES_OFFSET)[index] = 0x21;
...
const uint32_t  bits)
{
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = (*(SERIAL2 + SERIAL_CONFIG_OFFSET) & ~mask) | bits;
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = (*(SERIAL2 + SERIAL_CONFIG_OFFSET) & ~mask) | bits;
}
...
{
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = (*(SERIAL2 + SERIAL_CONFIG_OFFSET) & ~mask_bits[0]) | mask_bits[1];
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = (*(SERIAL2 + SERIAL_CONFIG_OFFSET) & ~mask_bits[0]) | mask_bits[1];
}
...
__attribute__((noinline)) void call_mskd_ref()
{
    ...
    &= ~SERIAL_CONFIG_POLARITY          ;  // clear for periph_reg_bits_val
}
...
__attribute__((noinline)) void call_periph_reg_bits_ref()
{
    ...
    periph_reg_bits_ref(SERIAL2, SERIAL2 + SERIAL_CONFIG_OFFSET, &polarity);
}
...
const    uint32_t    bits  )
{
    ...
    CHG_BITS(*(serial + SERIAL_CONFIG_OFFSET), SERIAL_CONFIG_DATALEN_MSK, bits);
...
{
    uint32_t    mask = SERIAL_CONFIG_DATALEN_MSK,
                bits = SERIAL_CONFIG_DATALEN_32_BITS;

    periph_reg_mskd_ref(SERIAL2, SERIAL2 + SERIAL_CONFIG_OFFSET, &mask, &bits);
}
...
__attribute__((noinline)) void call_return_bits()
{
    ...
    if (config_bits == SERIAL_CONFIG_ENDIAN)
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 83 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 89 << TIMER_PRESCALE_PRESCALER_POS;
}
...
__attribute__((noinline)) void call_return_mskd()
{
    ...
    if (mask == SERIAL_CONFIG_PARITY_MSK && bits == SERIAL_CONFIG_PARITY_EVEN)
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 157 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 163 << TIMER_PRESCALE_PRESCALER_POS;
}
...
__attribute__((noinline)) void call_return_reg()
{
    ...
    if (*config == SERIAL_CONFIG_POLARITY)
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 53 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 59 << TIMER_PRESCALE_PRESCALER_POS;
}
...
__attribute__((noinline)) void call_return_periph()
{
    ...
    if (*(serial + SERIAL_CONFIG_OFFSET) == SERIAL_CONFIG_POLARITY)
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 199 << TIMER_PRESCALE_PRESCALER_POS;
    else
        *(TIMER1 + TIMER_PRESCALE_OFFSET)  = 211 << TIMER_PRESCALE_PRESCALER_POS;
}
...
const uint8_t       pos)
{
    if (pos == SERIAL_CONFIG_ENDIAN_POS)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 0x33;
    else if (pos == SERIAL_CONFIG_POLARITY_POS)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 0x44;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 0x55;
}
...
{
    if (*pos == SERIAL_CONFIG_ENDIAN_POS)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 0x66;
    else if (*pos == SERIAL_CONFIG_POLARITY_POS)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 0x77;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 0x88;
}
...
const unsigned  port)
{
    if (port <= GPIO_MAX_PORT_NUM)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 127;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 131;
}
...
{
    if (port <= GPIO_MAX_PORT_NUM)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 149;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 151;
}
...
const unsigned  bit_num)
{
    if (bit_num <= sizeof(uint32_t) * 4 - 1)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 157;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 163;
}
...
{
    if (bit_num <= sizeof(uint32_t) * 4 - 1)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 167;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 173;
}
...
unsigned port)
{
    if (port <= SERIAL_MAX_PORT_NUM)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 179;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 181;
}
...
{
    if (port <= SERIAL_MAX_PORT_NUM)
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 191;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 193;
}
...
{
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = SERIAL_CONFIG_POLARITY | SERIAL_CONFIG_ENDIAN;

    uint32_t    extracted = SERIAL_CONFIG_MODE;

    extracted = *(SERIAL2 + SERIAL_CONFIG_OFFSET);

    if (extracted == (SERIAL_CONFIG_POLARITY | SERIAL_CONFIG_ENDIAN))
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 199;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 227;

    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = SERIAL_CONFIG_MODE;
    extracted       = *(SERIAL2 + SERIAL_CONFIG_OFFSET) ;
...
{
    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = 23 << SERIAL_CONFIG_RXPORT_POS;

    uint32_t                            bits = 11 << SERIAL_CONFIG_RXPORT_POS,
             __attribute__((unused))    mask =       SERIAL_CONFIG_RXPORT_MSK;

    bits = *(SERIAL2 + SERIAL_CONFIG_OFFSET) & SERIAL_CONFIG_RXPORT_MSK;

    if (bits == (23 << SERIAL_CONFIG_RXPORT_POS))
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 229;
    else
        (GPIO1 + GPIO_BYTES_OFFSET)[3] = 233;

    *(SERIAL2 + SERIAL_CONFIG_OFFSET) = 7 << SERIAL_CONFIG_RXPORT_POS;

    bits = *(SERIAL2 + SERIAL_CONFIG_OFFSET) & SERIAL_CONFIG_RXPORT_MSK;

    if (bits != (7 << SERIAL_CONFIG_RXPORT_POS))
        *(SERIAL2 + SERIAL_CONFIG_OFFSET) = SERIAL_CONFIG_PARITY_CRC;
    else
        *(SERIAL2 + SERIAL_CONFIG_OFFSET) = SERIAL_CONFIG_ENDIAN;
}
...
unsigned    test_num)
{
    unittest_record_registers(test_num,
                              ...
                              *(TIMER1  + TIMER_PRESCALE_OFFSET)   ,
                               (GPIO1   + GPIO_BYTES_OFFSET    )[3],
                              *(GPIO1   + GPIO_SET_OFFSET      )   );
